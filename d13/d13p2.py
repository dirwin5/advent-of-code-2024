...
def main():
    ...
    for line in lines:
        ...
        if line.startswith('Prize'):
            machine['Prize'] = (int(line.split('=')[1].split(',')[0]) + 10000000000000,
                                int(line.split('=')[2]) + 10000000000000)
    ...
    for machine in machines:
        ...
        if an % 1 == 0:
            ...
            print(f'Machine {machine}')
            print(f'an = {int(an)}, bn = {int(bn)}')
        else:
            print(f'Machine {machine}')
            print(f"an not an int. This machine hasn't been counted. an = {an}, bn = {bn}")

    print(f'Token total = {int(tokens_total)}')
